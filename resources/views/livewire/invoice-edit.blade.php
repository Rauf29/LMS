<div>
    ...
    @if($enableAddItem)
        <form wire:submit.prevent="saveNewItem">
            ...
            <div class="flex">
                ...
                <button class="lms-btn bg-red-600 ml-4" wire:click="addNewItem" type="button">Cancel</button>
            </div>
        </form>
    @else
        <button class="lms-btn mt-4" wire:click="addNewItem" class="underline">Add</button>
    @endif
    ...
    <ul class="mb-4">
        @foreach($invoice->payments as $payment)
            <li>{{date('F j, Y - g:i:a', strtotime($payment->created_at))}} - ${{number_format($payment->amount, 2)}}
                - transaction ID: {{$payment->transaction_id}} <button wire:click="refund({{$payment->id}})" class="bg-red-500 text-white px-2 text-xs">Refund</button>
            </li>
        @endforeach
    </ul>
</div>
